...
export function ShippingPage() {
  ...
  const createAccount = async (event) => {
    ...
    await run(async () => {
      ...
      setForm(initialForm);
      notify('success', 'Kargo API hesabi kaydedildi.');
      await load();
  ...
  return (
    <>
      ...
      {loading && accounts.length === 0 ? <LoadingState /> : (
        <>
          <section className="panel">
            <h2>Kargo Firmalari ve API Ayarlari</h2>
            <DataTable
              rows={accounts}
              columns={[
                ...
                { key: 'status', label: 'Durum', render: (row) => <span className={`badge ${row.status}`}>{row.status}</span> },
                {
                  ...
                  render: (row) => (
                    <div className="row-actions">
                      ...
                      <button type="button" onClick={() => action(row.id, api.shipping.retry, 'Retry kuyruga alindi.')} disabled={row.status !== 'failed'}><RotateCcw size={15} /> Retry</button>
