...
class UpdateHepsiburadaPriceInventoryJob implements ShouldQueue
{
    use Queueable;
    ...
    {
        return [(new WithoutOverlapping("hepsiburada-price:{$this->account->id}"))->expireAfter(900)->dontRelease()];
    }
    ...
    public function handle(HepsiburadaService $service, SyncRunService $runs): void
    {
        ...
        Log::info('Hepsiburada price inventory sync completed', ['account_id' => $this->account->id, 'duration_ms' => (int) ((microtime(true) - $startedAt) * 1000)]);
