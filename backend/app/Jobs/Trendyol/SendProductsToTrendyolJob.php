...
class SendProductsToTrendyolJob implements ShouldQueue
{
    ...
    {
        return [(new WithoutOverlapping("trendyol-products:{$this->account->id}"))->expireAfter(900)->dontRelease()];
    }
    ...
    public function handle(TrendyolService $service, SyncRunService $runs): void
    {
        ...
        Log::info('Trendyol product sync completed', ['account_id' => $this->account->id, 'duration_ms' => (int) ((microtime(true) - $startedAt) * 1000)]);
